...
/**
 * Renders one named boolean check as a wrapped title box, matching the
 * RSpec-style output some exercises use (my_css_is_easy_i and friends):
 *
 *   MY CSS IS EASY I IS THERE A    SUCCESS
 *   INDEX HTML AND A STYLE CSS?
 *
 * This is an approximation, not a byte-exact reproduction -- the real
 * output's exact wrap width/columns aren't independently knowable from a
 * single transcript, so this uses a reasonable fixed wrap width instead.
 */
function renderNamedCheck(title, passed) {
  const wrapWidth = 28;
  ...
  for (const w of words) {
    ...
    if (next.length > wrapWidth && current) {
      lines.push(current);
      current = w;
    } else {
      current = next;
  ...
  const w2 = status.length;

  return lines
    .map((line, i) => {
      const titleCell = "  " + center(line, wrapWidth);
      const statusCell = i === 0 ? statusColor(center(status, w2)) : " ".repeat(w2);
      return titleCell + "    " + statusCell + " ";
    })
    .join("\n");
}

module.exports = { render, scoreBar, renderTestCase, raw, renderNamedCheck };
