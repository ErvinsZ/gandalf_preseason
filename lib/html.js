...
const NAMED = { red: [255, 0, 0], black: [0, 0, 0], white: [255, 255, 255], green: [0, 128, 0], lime: [0, 255, 0] };
 ...
 */
function runScripts(scripts, { timeout = 2000, extraGlobals = {} } = {}) {
  const vm = require("vm");
  const logs = [];
  ...
  /**
   * Calls a global function directly and returns its actual value (and type),
   * rather than inferring it from printed text -- so `return 7` and
   * `console.log("7")` are told apart even though they print identically.
   */
  function callFunction(name, args = []) {
    try {
      const argsSrc = args.map((a) => JSON.stringify(a)).join(",");
      const probe = new vm.Script(
        `(function(){ try { if (typeof ${name} !== "function") return { ok: false, reason: "not-a-function" }; ` +
          `return { ok: true, value: ${name}(${argsSrc}) }; } ` +
          `catch (e) { return { ok: false, reason: "threw", message: e.message }; } })()`
      );
      return probe.runInContext(context);
    } catch (e) {
      return { ok: false, reason: "threw", message: e.message };
    }
  }

  return { context, logs, error, getGlobal, callFunction };
}

/** "background-color: red; right: 70" -> { backgroundColor: "red", right: "70" } */
...
/** True for any zero-length CSS value: "0", "0px", " 0.0px", etc. */
...
/**
 * A minimal `document` for scripts that call `getElementById(id).style.xxx = ...`.
 * Returns { document, elements } -- `elements[id]` exists once the script has
 * asked for that id, so callers can inspect what was set on `.style` after
 * running.
 */
function makeDomStub() {
  ...
  const document = {
    getElementById(id) {
      if (!elements[id]) elements[id] = { id, style: {} };
      return elements[id];
    },
  };
...
/**
 * Replaces the first top-level `varName = <literal>;` assignment with
 * `varName = newLiteral;`. Used to re-run a fixed exercise template with
 * different input values, so a hardcoded/lucky-guess answer that only works
 * for the example in the instructions gets caught. Returns null if no such
 * assignment could be found (e.g. the student renamed or restructured it).
 */
function overrideAssignment(source, varName, newLiteral) {
  const re = new RegExp(`\\b${varName}\\b\\s*=\\s*[^;\\n]+;`);
  if (!re.test(source)) return null;
  return source.replace(re, `${varName} = ${newLiteral};`);
}

/** Applies several overrideAssignment calls in sequence; null if any fails. */
function overrideMany(source, overrides) {
  let out = source;
  for (const [name, value] of Object.entries(overrides)) {
    const next = overrideAssignment(out, name, value);
...
module.exports = {
  ...
  makeDomStub, overrideAssignment, overrideMany, parseInlineStyle, isZeroLength,
};
